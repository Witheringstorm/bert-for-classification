...
def main():
    ...
    if all_texts_cn and all_labels_cn:
        ...
        min_samples_for_stratify = 2 * len(set(all_labels_cn)) # At least 2 samples per class
        can_stratify = (len(set(all_labels_cn)) > 1 and 
                        all(pd.Series(all_labels_cn).value_counts() >= min_samples_for_stratify // len(set(all_labels_cn))))

        train_texts_cn, test_texts_cn, train_labels_cn, test_labels_cn = train_test_split(
            all_texts_cn, all_labels_cn, test_size=0.2, random_state=42, 
            stratify=all_labels_cn if can_stratify else None
        )
        print(f"Chinese data: {len(train_texts_cn)} train, {len(test_texts_cn)} test examples.")
